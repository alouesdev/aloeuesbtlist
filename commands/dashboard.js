...
if(args[1] == "aç") {
    ...
    .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
    .setThumbnail(client.user.avatarURL())
    .setDescription("Gliot Botlist **bakım** modu aktif edildi!")
    .addField("Yetkili", message.author)
    .addField("Sebep", reason)
    .setColor("BLUE")
    client.channels.cache.get(ch.webstatus).send(`Gliot Botlist has been switched to __maintance__ due to **${reason}**`).then(a => { 
...
if(args[1] == "kapat") {
    ...
    .setThumbnail(client.user.avatarURL())
    .setDescription(`Tebrikler, **${d.reason}** sebebiyle kapanmış olan Gliot Botlist'u tekrar açtınız.`)
    .setColor("GREEN")
    .setTitle("BAŞARILI")
    message.channel.send(tebriklerkanks);
    client.channels.cache.get(ch.webstatus).messages.fetch(d.bakimmsg).then(a => { a.edit(`~~ Gliot Botlist **${d.reason}** sebebi ile __bakım__ moduna alındı. ~~`, bakimbitti) } )
    const bakimbitti = new Discord.MessageEmbed()
    .setAuthor("www.rabelcode.xyz", client.user.avatarURL())
    .setThumbnail(client.user.avatarURL())
    .setColor("GREEN")
    .setDescription(`Gliot Botlist are **active** again!\n[Click to redirect website](https://www.rabelcode.xyz)`)
    .setFooter("Gliot Botlist © Tüm hakları saklıdır.");
